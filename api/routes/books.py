...
async def get_book(book_id: int) -> Book:
    try:
        book: Optional[Book] = db.get_book(book_id)  # Assumes db has this method
    ...
    except Exception as e:  # Catch other potential exceptions (database errors, etc.)
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Error fetching book: {e}")
